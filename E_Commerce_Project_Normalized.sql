...
/*
e_commerce_data.csv file has been uploaded to E-Commerce database

*/

-- Cleaning Order_id, Customer_id, Shipping_Id and turned into integer
...
---- Creating Orders column and eliminating dublicate entries and adding neccessary constraints for normalization
...
-- Orders and Shipping tables giving references
ALTER TABLE orders
ALTER COLUMN [ship_id] INT NOT NULL
GO
...
--- Dropping first e_commerce table
...
(
	...
	FROM orders
	WHERE Prod_ID=11)
),    -- Total quantity of products 11-14 ordered by each customer
T2 AS 
(
...
SELECT DISTINCT t1.Cust_ID, Total_11_14, Total_Quantity_by_Customer,   -- ratio of order quantities of 11-14 over total ordered quantity by each customer
